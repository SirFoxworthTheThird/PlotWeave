...
import { sceneDrafts } from './moby-dick/full-scene-drafts.mjs'
...
const lorePages=[
 ['source','Complete Source and Editorial Calendar','The structure and manuscript follow Project Gutenberg edition #2701: the complete narrative of all 135 author-titled chapters and the Epilogue in source order. Melville’s Etymology and Extracts, Gutenberg front matter, and Gutenberg end matter are excluded as paratext. The voyage’s exact dates are often unstated, so the calendar begins with Ishmael’s winter departure and uses an explicitly editorial sequence of elapsed days to preserve order rather than claiming historical precision.',1],
...
const sceneTexts=events.map((event,index)=>{const text=sceneDrafts[index];return{...base,id:id('scene',String(index+1).padStart(3,'0')),eventId:event.id,text,wordCount:text.trim().split(/\s+/u).length}})
...
const entry={id:'moby-dick',title:'Moby-Dick; or, The Whale',author:'Herman Melville',blurb:'Ishmael joins the Pequod, where Captain Ahab turns a whaling voyage into a consuming hunt for the white sperm whale that maimed him.',data:'moby-dick.pwk',dataBytes:Buffer.byteLength(text),counts:{characters:characters.length,chapters:chapters.length,events:events.length,locations:locations.length},notice:'Unofficial reference for a public-domain novel. Manuscript scenes reproduce the complete narrative of all 135 chapters and the Epilogue from Project Gutenberg eBook 2701; Etymology, Extracts, Gutenberg front matter, and end matter are excluded.',worldId,cover:'library/moby-dick/art/world.png'}
