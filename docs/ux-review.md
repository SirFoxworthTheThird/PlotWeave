...
| ID | Severity | Status | Finding |
|---|---|---|---|
| NEW-1 | high | **fixed** | **The full 14-icon nav rail is present during onboarding**, so every one of a dozen empty screens is one click away from a flow that is trying to guide you. The guide and the freedom undercut each other. **Fixed, narrower than filed.** The dozen empty screens are exactly the rail's extended tier — Corkboard, Calendar, Structure, Items, Relations, Arc, Lore, Factions, Knowledge, Settings — and those go away while the guide is up. The five core routes stay: 33 e2e specs leave a blank world by the rail, which is evidence that doing so is the normal path, and taking navigation away entirely would make the guide modal in all but name. |
