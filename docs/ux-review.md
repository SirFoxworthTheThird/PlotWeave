...
| HB-2d | med | **open** | **On a phone the hover-revealed deletes are drawn at full strength and take no taps.** Found while correcting HB-2b, because both rest on the same rule. `index.css` forces `[class~="opacity-0"][class*="group-hover:opacity-"]` to `opacity: 1 !important` on a hover-less pointer, and that lifts only the *opacity* half of the gate — the `pointer-events: none` half stays. Measured on the world card at 390×667 with touch on: `opacity: 1`, `pointer-events: none`, and a tap on it leaves the world exactly where it was. Seven controls share the shape (LORE-1's list), including the one that deletes a world. This is not the hazard LORE-1 guarded against — nothing fires by accident, and on that count the gate is doing its job — it is the opposite one: a control that is fully drawn, indistinguishable from a working button, and inert. **It may well be deliberate, and the markup is the argument for that.** The gate sits on the delete button alone, not on the row around it — the lesson of the earlier mistake where `pointer-events-none` on the whole action row took Export down with it. So on a phone the world card's Export *is* tappable and its Delete is not: the row is not dead, one control in it is, and it is the destructive one. Read that way this is "no deleting from a list on touch", which is a defensible rule. Whether the answer is to say so, to lift `pointer-events` with the opacity (restoring the accidental-tap risk on the device where it is worst), or to hide these outright on touch, is a design decision rather than a class, so it is filed rather than guessed. What is *not* left to chance is the description: `e2e/hoverRevealSafety.spec.ts` now asserts both halves on a phone, so the behaviour is written down wherever it ends up. |
