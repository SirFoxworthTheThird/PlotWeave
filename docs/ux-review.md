...
| MW-9 | med | **open — groundwork done, four of sixteen layers read** | **Thirteen map scales in the Fellowship example are unverified.** Two of the fifteen have been checked against their own printed bars — Middle Earth was wrong by 3.7× and in the wrong unit, the Shire was right — so neither "they are all fine" nor "convert them all" is safe. Each remaining layer needs its artwork read: some may carry no bar at all, in which case the honest answer is to clear the scale rather than assert a number nothing backs. | **Not fixed, but no longer unmeasured.** Three things were established, and they change the size of the job. **(1) An arithmetic screen costs nothing and finds the worst cases.** `imageWidth / scalePixelsPerUnit` is the extent the stored scale claims, and against the depicted place it is decisive: Hobbiton claims 1.9 km and Bree 2.7 km, which are right for a village and a town — while **Isengard claims 2,595 km**, **Endoras 1,138 km**, **Minas Tirith 1,022 km** and **Moria 813 km**, for a fortress ring, a town, a city and a mine. Those four are wrong by two to three orders of magnitude whatever their artwork says. **(2) The artwork does carry bars, in three different units.** Minas Tirith's reads *Feet*, Middle Earth's reads *Miles* (the root map, stored as `km` — the review's "wrong unit" confirmed independently), and Moria's reads *A scale of twenty leagues*. Endoras has a bar marked 200/400/600; Isengard's is not in the corner the search looked in. **(3) Middle Earth, Lothlórien Journey and The Lower Anduin are the same image** (`kImOXlIVVkxowBRHH8KbM`), so their identical scale is correct by construction rather than a copy-paste — one number to establish, not three. **One layer is measured end to end**, as a worked example of the method: Minas Tirith's bar spans 325.7 px for 600 feet (ticks located by column-darkness profile between the rules, not by eye), giving **1,781 px/km** against the stored 1.7611 — and the cross-check holds, since 1800 px at that scale is 1.01 km, which is the city's size. **Deliberately not applied.** Correcting one of sixteen leaves the set incoherent and gives nothing to test against; the value is in doing the remaining images the same way and then asserting, in one go, that no shipped layer claims an extent its own subject contradicts. |
