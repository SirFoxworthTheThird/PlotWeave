...
|---|---|---|
| HB-1 | **Adding a scene participant gives no path to the snapshot it needs.** | **Half fixed, half real.** The mislabelling half is gone: `charactersNotInChapter` (added after their pass) starts the panel from the scene's own cast, so an assigned character shows under their scene as *"in the scene, no state recorded"* and the folded group reads *"N **other** characters **not in this chapter**"* — their recommendation 2, already done. The action half stands: **1 of 28** issue kinds in `computeIssues.ts` carries a one-click `fix`, and `char-before-intro` only navigates to the chapter. |
