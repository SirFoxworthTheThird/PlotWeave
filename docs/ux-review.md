...
| SEL-3a | low | open | **An imported world gets no cover image.** Split out of **SEL-3**, whose card half is fixed. The card renders `world.coverImageId` and falls back to a globe, so the card is not the problem — nothing assigns a cover when a world arrives by import, even when the import carried images. |
...
| MW-9 | med | open | **Thirteen map scales in the Fellowship example are unverified.** Two of the fifteen have been checked against their own printed bars — Middle Earth was wrong by 3.7× and in the wrong unit, the Shire was right — so neither "they are all fine" nor "convert them all" is safe. Each remaining layer needs its artwork read: some may carry no bar at all, in which case the honest answer is to clear the scale rather than assert a number nothing backs. |
...
| MT-6 | med | **fixed, partly corrected** | **Sync points only work one way, and only while playing.** They fire when playback advances the **inner** track onto a paired event, moving a hidden outer cursor that only the **map** reads (for ghost pins). Playing the outer track syncs nothing, and moving the cursor by hand syncs nothing in either direction. A writer who pairs nine moments and then scrubs between them sees no effect at all. | **The "only while playing" half is fixed.** The sync was applied inside the playback timer's callback, so it fired only when the timer moved the cursor. It is an effect on the cursor now (`src/lib/syncPoints.ts`), so it follows every way the cursor moves — the timer, a click on the scrubber, the previous/next arrows, the search palette. It also **holds** the last pairing rather than matching exactly, which is what a frame narrative means: the teller stays at that point in the telling until the story reaches the moment that moves them. Exact matching would have shown the frame's cast on one scene and dropped them on the next. **The "one way" half is the design, not a fault.** The outer cursor exists to draw the frame's cast as ghost pins beside the inner story — something you want while you are *in* the tale — and has no consumer in the other direction, so a symmetrical sync would set a value nothing reads. **Not covered end-to-end, and worth saying why:** ghost pins are the only surface `activeOuterEventId` has, and a purpose-built frame narrative would not produce any — see **MT-8**. The wiring was confirmed by instrumenting the running app during development (scrubbing to a paired moment set the outer cursor and the map resolved its title), but an assertion that survives is another matter, so what ships is the unit tests over `outerEventAt`. |
| MT-8 | med | open | **Ghost pins could not be produced in a purpose-built frame narrative.** Building one from scratch — two timelines, a frame relationship, a sync point, an outer character with a snapshot placing them at a marker on the current layer — the map resolved the paired outer moment correctly (`activeOuterEventId` set, `outerActiveEvent.title` right) and still selected **zero** outer snapshots, so no ghosts were drawn. Instrumented far enough to rule out the obvious: the outer chapter was found, the sync fired, the title resolved, and distinct chapter numbering across the two timelines made no difference. **Deliberately filed as an observation rather than a defect** — it may still be something missing from the fixture rather than from `selectBestCharacterSnapshots`, and I did not isolate it. Whoever picks it up should start by building the same world through the UI and seeing whether ghosts appear at all; if they do not, the feature is unreachable and that is a high finding rather than a medium one. |
