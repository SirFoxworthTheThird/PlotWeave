...
**Status: Phase 2 complete (management UI). Phase 3 (playback scoping) not yet started.**
...
- Clicking opens a `TimelineRelationshipPanel` slide-over showing existing relationships and a "Link Timelines" form.
- Form fields: source timeline, target timeline, type (with a one-line description of each), label, description.
- Each relationship card expands to reveal:
  - **Anchors** — type (character/location/document) + entity picker; add and remove individually.
  - **Sync points** (frame narrative only) — inner event → outer event picker table; add and remove individually.
...
| `src/lib/ghostMarkerIcon.ts` | Ghost pin `DivIcon` factory (Phase 4) |
...
| `src/db/hooks/useSnapshots.ts` | Optional `timelineEventIds` scope parameter (Phase 3) |
...
| `src/components/ChapterTimelineBar.tsx` | Replace `timelines[0]`; stacked depth tracks; sync point firing; per-track play buttons (Phase 3) |
| `src/features/timeline/TimelineView.tsx` | "Link Timelines" button (2+ timelines gate) |
| `src/features/worlds/WorldDashboardView.tsx` | Relationship count in timeline tile description |
| `src/features/maps/MapExplorerView.tsx` | `playbackTimelineId` scoping; timeline-filtered `orderedEvents`; ghost pins; echo markers (Phase 3+) |
| `src/features/maps/MapTimeline.tsx` | Replace `timelines[0]` (Phase 3) |
| `src/features/maps/LeafletMapCanvas.tsx` | `ghostPins` and `echoMarkers` props and render layers (Phase 4+) |
| `src/features/items/ItemDetailView.tsx` | "Cross-timeline appearances" section (Phase 6) |
| `src/features/continuity/ContinuityChecker.tsx` | Artifact anachronism check (Phase 6) |
| `src/features/arc/CharacterArcView.tsx` | Scope arc to active playback timeline (Phase 3) |
| `src/features/characters/tabs/HistoryTab.tsx` | Annotate snapshots from counterpart timelines (Phase 6) |
...
- [x] `TimelineRelationshipPanel.tsx` — list, create, edit, delete relationships; anchor editor; sync point editor (inner event → outer event picker, per relationship)
- [x] `TimelineView.tsx` — "Link Timelines" button (2+ timelines gate)
- [x] `WorldDashboardView.tsx` — relationship count in timeline tile description
