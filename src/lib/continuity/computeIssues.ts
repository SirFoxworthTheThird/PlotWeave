...
export function computeContinuityIssues(input: ContinuityInput): Issue[] {
  const {
    ...
    */
    ...
    /*
      One object in two places at one scene.

      A blind run tried four times to make this fire through the UI and could
      not, and filed it as possibly dead code. It is not, but the reason it is
      hard to reach by hand is worth writing down: every write path deliberately
      prevents the contradiction rather than recording it. The Current State
      hand-off and the map panel's take the item off the previous holder, and
      `placeItemAtLocation` does the same when an item is put down. So what
      reaches this rule is a world that arrived already contradicting itself —
      an import, a merge, or a chapter generated by AI — which is exactly the
      case a writer cannot check by eye.

      It compares the records literally present at one scene. A holder recorded
      chapters earlier, with nothing since, is not compared: state resolves by
      last known, and this does not resolve. That was invisible while a write
      path had the same blind spot; it no longer does, so what remains is a
      limit on what this rule sees, not a way to produce what it misses.
    */
