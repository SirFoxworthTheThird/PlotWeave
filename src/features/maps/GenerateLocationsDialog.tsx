...
function buildPrompt(existingTree: string): string {
  const existingBlock = existingTree
    ? `
PLACES ALREADY IN THIS WORLD (indentation shows nesting) — do NOT recreate these; only ADD new places or nest new children under one of them. To add under an existing place, reuse its EXACT name as a parent and put the new place in its "children" (a matching name is updated in place, never duplicated). A line in [square brackets] is a FLOOR/LEVEL of the place above it (e.g. "[Ground floor]"); the places indented under it live on that floor — reuse those exact names, and treat the floor as part of that place:
${existingTree}
`
    : ''
...
SHAPE:
{
  ...
  "locations": [
    ...
    {
      "name": "<a multi-storey place, e.g. a castle>",
      "type": "building",
      "levels": [
        { "name": "Ground floor", "children": [ { "name": "<room on that floor>" } ] },
        { "name": "First floor", "children": [ { "name": "<room on that floor>" } ] }
      ]
    }
  ]
}
...
- "levels" is ONLY for a place that is one footprint stacked into FLOORS you switch between — a castle, tower, keep, ship, or multi-storey building. Each level has a "name" (e.g. "Dungeons", "Ground floor", "First floor") and its own "children" (the rooms/locations on that floor), ordered bottom to top. Use "levels" OR "children" on a place, not both. Most outdoor/geographic places use "children", not "levels".
