...
function RelationshipCard({ rel, timelines, allEvents, allChapters, characters, locations, items }: CardProps) {
  ...
  const innerOptions = useMemo(() => makeEventOptions(rel.targetTimelineId), [allEvents, allChapters, rel.targetTimelineId])
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const outerOptions = useMemo(() => makeEventOptions(rel.sourceTimelineId), [allEvents, allChapters, rel.sourceTimelineId])
