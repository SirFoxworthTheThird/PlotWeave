...
A visual story-tracking web app for writers who need to keep track of characters, locations, relationships, events, and world lore across their stories — built to be used alongside tools like Word or Scrivener.
...
*Each character has five tabs: Overview (bio), Current State (chapter snapshot — location, inventory, status), History (all snapshots across chapters), Relationships, and Lore (linked lore pages).*
...
- **Lore tab** — lists all lore pages linked to this character; click any card to open it in the lore editor
...
- **Related lore** — a Related Lore section on the item detail page lists all lore pages linked to that item

### Lore
- **World knowledge base** — a dedicated wiki-style section for documenting magic systems, history, factions, languages, and anything that lives outside the timeline
- **Categories** — colour-coded categories organised in a sidebar; pages without a category appear under Uncategorised
- **Markdown editor** — full-page editor with preview toggle, auto-save, and a tag bar in the footer
- **Revealed at** — each page can be pinned to the exact timeline event when that lore is revealed in the story; the **Revealed** filter on the index then shows only what the reader knows so far
- **Entity linking** — link a page to any character, item, or location marker; linked pages appear in the **Lore tab** on the character/item detail page and in the Writer's Brief when those entities are present
- **Brief integration** — the Writer's Brief shows relevant lore pages alongside characters; pages revealed at the current event are marked **NEW**
- **Search** — lore page titles, body text, and tags are included in the global search palette

### Global Search
- **Command palette** — press `Ctrl+K` (or `⌘K` on Mac) from anywhere in the app to open a full-text search palette
- **Searches everything** — characters, items, location markers, chapters, events, timelines, relationships, and lore pages all searched simultaneously
...
- **At a glance** — displays the chapter title and synopsis, all events, each character's current location / alive status / status notes / inventory, all relationship states recorded for that chapter, every item placed at a location, and relevant lore pages
- **Lore in the brief** — shows pages linked to present characters plus any page revealed at the current event (tagged NEW); click to open the lore editor
...
### Export / Import
- **`.pwk` format** — export any world to a single JSON file containing all data and base64-encoded images (current format: v5)
- **Full fidelity** — characters, maps, location markers, timelines, chapters, events, items, relationships, snapshots, movement paths, images, lore pages, lore categories, and relationship graph positions are all included
- **Backward compatible** — older `.pwk` files missing newer fields are normalised on import; smart merge mode keeps the newer record per entity when merging two exports
...
11. **Write lore pages** — open the Lore section to document magic systems, history, factions, and more; link pages to characters and items so they surface in the right places
12. **Play the Story So Far** — press Play in the timeline bar to animate your characters through all chapters with cinematic notes
13. **Compare chapters** — click the compare icon in the timeline bar to diff any two chapters and see exactly what changed
14. **Open the Writer's Brief** — click the scroll icon in the top-right for a full summary of the active chapter, including relevant lore
15. **Search your world** — press `Ctrl+K` to find any character, location, chapter, item, or lore page instantly
